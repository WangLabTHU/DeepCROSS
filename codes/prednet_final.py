...
class PredictorNet_language:
    ...
    def train(self, dataset, labels, savepath, ratio=0.9, transfer=False, modelpath=None):
        ...
        if (transfer):
            ...
            print("de novo generating a new model!")
...
def linear_calibration(data_x, anchor_y, anchor_x):
    
    min_y, max_y = np.min(anchor_y), np.max(anchor_y)
    min_x, max_x = np.min(anchor_x), np.max(anchor_x)
    maps_y = []
    for item in data_x:
        new_y = (max_y - min_y) / (max_x - min_x) * (item - min_x) + min_y
        maps_y.append(new_y)
    return maps_y    

def myfilter(seqs, exprEC, exprPA, mode="EC"):
    
    N = int(len(seqs) * 0.9)
    if mode == "EC":
        high_idx = np.argsort(-exprEC)[:N]
    elif mode == "PA":
        high_idx = np.argsort(-exprPA)[:N]
    else:
        print("error mode")
        exit(0)

    random.seed(0)
    res_seqs, res_exprEC, res_exprPA = seqs[high_idx], exprEC[high_idx], exprPA[high_idx]
    
    idx = list(range(len(res_seqs)))
    random.shuffle(idx)
    res_seqs = np.array(res_seqs)[idx]
    res_exprEC = np.array(res_exprEC)[idx]
    res_exprPA = np.array(res_exprPA)[idx]
    
    return res_seqs, res_exprEC, res_exprPA

if __name__ == "__main__":
    
    '''
    step1: pretraining fine-tuning datasets
    '''
    
    ## 2024
    df_mpra2024 = pd.read_csv("../dataset/AAE_supervised/calibration_mpra2024.csv")
    seqs =  list(df_mpra2024.loc[:,"seqs"])
    expr_ec = list(df_mpra2024.loc[:,"EC(2024)"])
    expr_pa = list(df_mpra2024.loc[:,"PA(2024)"])
    
    random.seed(0)
    idx = list(range(len(seqs)))
    random.shuffle(idx)
    seqs = np.array(seqs)[idx]
    expr_ec = np.array(expr_ec)[idx]
    expr_pa = np.array(expr_pa)[idx]
    
    write_fa("../dataset/PredNet_final/seqs.txt", seqs)
    write_exp("../dataset/PredNet_final/expr_ec.txt", expr_ec)
    write_exp("../dataset/PredNet_final/expr_pa.txt", expr_pa)
    
    
    
    '''
    step2: training models with pretrained strategies, here the col4_PretrainEC come from the pretraining on `replicate_fig3/col4` data,
    which has been filtered for top 90%
    '''
    
    pretrained_model = "../figs/replicate_fig3/col4_PretrainEC/checkpoint.pth"
    model = PredictorNet_language(model_name = "pred_ec_final", length=165, patience=10)
    dataset = "../dataset/PredNet_final/seqs.txt"
    labels_ec  = "../dataset/PredNet_final/expr_ec.txt"
    save_path = "./check/"
    model.train(dataset=dataset,labels=labels_ec,savepath=save_path, transfer=True, modelpath=pretrained_model)
    
    pretrained_model = "../figs/replicate_fig3/col4_PretrainPA/checkpoint.pth"
    model = PredictorNet_language(model_name = "pred_pa_final", length=165, patience=10)
    dataset = "../dataset/PredNet_final/seqs.txt"
    labels_pa  = "../dataset/PredNet_final/expr_pa.txt"
    save_path = "./check/"
    model.train(dataset=dataset,labels=labels_pa,savepath=save_path, transfer=True, modelpath=pretrained_model)
    ...
    model = PredictorNet_language(model_name = "pred_ec_final", length=165)
    model_ec = "./check/pred_ec_final/checkpoint.pth"
    seqs = open_fa("../dataset/PredNet_final/seqs.txt")
    label_ec = open_exp("../dataset/PredNet_final/expr_ec.txt", operator="direct")
    pred_ec_final = model.predict_input(model_path=model_ec, inputs=seqs, mode="data")
    
    model = PredictorNet_language(model_name = "pred_pa_final", length=165)
    model_pa = "./check/pred_pa_final/checkpoint.pth"
    seqs = open_fa("../dataset/PredNet_final/seqs.txt")
    label_pa = open_exp("../dataset/PredNet_final/expr_pa.txt", operator="direct")
    pred_pa_final = model.predict_input(model_path=model_pa, inputs=seqs, mode="data")
    
    plot_reg(label_ec, pred_ec_final, "../results/prediction_final/prednet_pcc_self_ec.png",c1="mediumpurple")
    plot_reg(label_pa, pred_pa_final, "../results/prediction_final/prednet_pcc_self_pa.png",c1="indianred")
    
    
    '''
    step4: prediction of Lib-1 results
    ...
    plot_reg(label_ec, pred_ec_final, "../results/prediction_final/prednet_pcc_lib1_ec.png",c1="mediumpurple")
    plot_reg(label_pa, pred_pa_final, "../results/prediction_final/prednet_pcc_lib1_pa.png",c1="indianred")
    
    '''
    step5: prediction of NM2018 data
    '''
    
    df_nm2018 = pd.read_csv("../dataset/AAE_supervised/calibration_nm2018.csv")
    seqs = list(df_nm2018.loc[:,"seqs"])
    label_ec = list(df_nm2018.loc[:,"EC(2018)"])
    label_pa = list(df_nm2018.loc[:,"PA(2018)"])
    ...
    plot_reg(label_ec, pred_ec_final, "../results/prediction_final/prednet_pcc_nm2018_ec.png",c1="mediumpurple")
    plot_reg(label_pa, pred_pa_final, "../results/prediction_final/prednet_pcc_nm2018_pa.png",c1="indianred")
    
    '''
    step6: prediction of Lib-2 results
    '''
    
    df_lib2 = pd.read_csv("../dataset/AAE_supervised/calibration_mpra_20240713.csv")
    seqs = list(df_lib2.loc[:,"seqs"])
    label_ec = list(df_lib2.loc[:,"EC(2024)"])
    label_pa = list(df_lib2.loc[:,"PA(2024)"])
    ...
    plot_reg(label_ec, pred_ec_final, "../results/prediction_final/prednet_pcc_lib2_ec.png",c1="mediumpurple")
    plot_reg(label_pa, pred_pa_final, "../results/prediction_final/prednet_pcc_lib2_pa.png",c1="indianred")
