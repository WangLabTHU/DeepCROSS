...
class CNN_minexp_value():
    def __init__(self):
        self.forward_lstm = layers.LSTM(75, return_sequences=True, unroll=True) # go_backwards 默认为false
        self.backward_lstm = layers.LSTM(75,go_backwards=True,return_sequences=True, unroll=True)
        # 注意前后lstm的 go_backwards 必须设置不同，一个为false一个为true
        self.brnn = layers.Bidirectional(self.forward_lstm,merge_mode='concat',backward_layer=self.backward_lstm)
    def PredictorNet(self, x, is_training=True, reuse=False):
        with tf.variable_scope("Predictor", reuse=reuse):
            ...
            x = Conv1D(filters=64,kernel_size=3,padding="valid",activation="relu")(x)#seqInput
