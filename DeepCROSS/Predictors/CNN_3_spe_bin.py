...
import os
from ..ProcessData import seq2oh,GetCharMap,load_fun_data,load_fun_data_exp3,oh2seq
#
import pickle
...
#        plot(self.val_y,pred,'expression')
    ...
    def predict_val(self,data=None):
        ...
        #4.预测是univ的序列里，有多少是确实univ的
        #predicted univ set
        #从第3个bin(即下面index是从0计数的>=2)开始认为是active的,此时log2exp>-5,略严,文章里只需要>=-6
        Index_univ_pred=np.where(np.sum(np.array(I_max_pred>=2,dtype='int'),axis=-1)==3)[0]
        pred_univ_onehot=self.val_x[Index_univ_pred,:,:]
        pred_univ_seq=set(oh2seq(pred_univ_onehot,self.invcharmap))
        #true univ set
        Index_univ_true=np.where(np.sum(np.array(I_max_true>=2,dtype='int'),axis=-1)==3)[0]
        true_univ_onehot=self.val_x[Index_univ_true,:,:]
        true_univ_seq=set(oh2seq(true_univ_onehot,self.invcharmap))
        correct_univ=true_univ_seq&pred_univ_seq
        #
        corretc_Index=set(Index_univ_pred)&set(Index_univ_true)
        Index_false_positive=list(set(Index_univ_pred)-corretc_Index)
        Index_false_negative=list(set(Index_univ_true)-corretc_Index)
        Index_univ_correct=list(corretc_Index)
        print('correct:')
        print(len(correct_univ))
        print('pred:')
        print(len(pred_univ_seq))
        print('true:')
        print(len(true_univ_seq))
        print('correct univ prop:')
        print(len(correct_univ)/len(pred_univ_seq))
        #
        #画7条来看看
        spe_list=['BS','EC','PA']
        color_list=['red','orange','blue']
        ...
        if len(np.where(y_true[0][0]==1)[0])!=1: print('Error:More than one place with 1')
        #'./exp_3_dir_bin_only5/'
        pdf = PdfPages(self.weight_dir+'exp_bin_true_pred_bin_univ_correct'+'.pdf') 
        numplot=15 #len(Index_false_positive)
        plt.figure(figsize=(21,numplot*7)) 
        for j in range(numplot): #j表示序列
            for i in range(len(spe_list)): #i表示物种
                spe=spe_list[i]
                #subplotI=int('73'+str(3*j+i+1))
                #plt.subplot(subplotI)#131 132 133
                plt.subplot2grid((numplot,3),(j,i))
                #true seqs(univ.pdf)
                #plt.bar(np.array(binlist[:-1]),y_pred[Index_univ_true[j]][i], align='edge',facecolor = color_list[i], width = 1.2,edgecolor = 'white',label='Pred_exp_'+spe)
                #plt.bar(np.array(binlist[:-1]),y_true[Index_univ_true[j]][i], align='edge',facecolor = 'green',alpha=0.3,width = 1.2,edgecolor = 'white',label='True_exp_'+spe)
                #predicted seqs(reverse.pdf)
                #plt.bar(np.array(binlist[:-1]),y_pred[Index_univ_pred[j]][i], align='edge',facecolor = color_list[i], width = 1.2,edgecolor = 'white',label='Pred_exp_'+spe)
                #plt.bar(np.array(binlist[:-1]),y_true[Index_univ_pred[j]][i], align='edge',facecolor = 'green',alpha=0.3,width = 1.2,edgecolor = 'white',label='True_exp_'+spe)
                plt.bar(np.array(binlist[:-1]),y_true[Index_univ_correct[j]][i], align='edge',facecolor = color_list[i],width = 1.2,edgecolor = 'white',label='True_exp_'+spe)
                plt.bar(np.array(binlist[:-1]),y_pred[Index_univ_correct[j]][i], align='edge',facecolor =(0,0,0,0),edgecolor ='black',width = 1.2,label='Pred_exp_'+spe)
                #false positive seqs(univ_fp.pdf)
                #plt.bar(np.array(binlist[:-1]),y_true[Index_false_positive[j]][i], align='edge',facecolor = color_list[i],width = 1.2,edgecolor = 'white',label='True_exp_'+spe)
                #plt.bar(np.array(binlist[:-1]),y_pred[Index_false_positive[j]][i], align='edge',facecolor =(0,0,0,0),edgecolor ='black',width = 1.2,label='Pred_exp_'+spe)
                #false negative seqs(univ_fn.pdf)
                #plt.bar(np.array(binlist[:-1]),y_true[Index_false_negative[j]][i], align='edge',facecolor = color_list[i],width = 1.2,edgecolor = 'white',label='True_exp_'+spe)
                #plt.bar(np.array(binlist[:-1]),y_pred[Index_false_negative[j]][i], align='edge',facecolor =(0,0,0,0),edgecolor ='black',width = 1.2,label='Pred_exp_'+spe)
                #correct
                #plt.bar(np.array(binlist[:-1]),y_true[Index_univ_pred[j]][i], align='edge',facecolor = color_list[i],width = 1.2,edgecolor = 'white',label='True_exp_'+spe)
                #plt.bar(np.array(binlist[:-1]),y_pred[Index_univ_pred[j]][i], align='edge',facecolor =(0,0,0,0),edgecolor ='black',width = 1.2,label='Pred_exp_'+spe)
                #
                plt.legend(loc='upper right', fontsize=10, frameon=False, fancybox=False, borderpad=0.3,ncol=1, markerfirst=True, markerscale=4, numpoints=1, handlelength=2)
                plt.xlabel('Promoter activity ($\mathregular{log_2}$)')
                plt.ylabel('Probability')
