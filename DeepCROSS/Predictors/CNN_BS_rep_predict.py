...
class rep_predict_exp():
    def __init__(self,
        ...
        self.x,self.y = load_fun_data_exp3(train_data,flag=1)
        ...
        if val_data != None:
            ...
            self.val_x, self.val_y = self.x[seq_index_A[n:],:], self.y[seq_index_A[n:]]
            self.x, self.y = self.x[d,:], self.y[d]
            self.x=self.x.reshape(-1,64,1) #np.swapaxes(self.x,0,1)
            self.val_x=self.val_x.reshape(-1,64,1) #np.swapaxes(self.val_x,0,1)
            print('x shape:')
            print(self.x.shape) #(64,11264*0.9)
        self.DIM = DIM
        self.kernel_size = 5
        self.model = self.BuildModel()

    def BuildModel(self):#cannot fit
        regW = regularizers.l2(0.00001)
        seqInput= Input(shape=(64,1,),name='seqInput') #shape=(self.seq_len,4)
    ...
    def BuildModel_overfitting(self):#overtiffing!LSTM+cxh
        regW = regularizers.l2(0.00001)
...
#        plot(self.val_y,pred,'expression')
    ...
    def Predictor(self,seq):
        ...
        y = self.model.predict(seq) #这里的seq是指rep,是序列在表示空间的向量
