...
class CLS_alone_attention():
    ...
    def out_rep2y(self,last2rep):
        ...
        cls_logits_EC =  Linear('Dense_label.3',self.d_model,3, last2rep)#self.nbin(3)
        cls_logits_PA =  Linear('Dense_label.4',self.d_model,3, last2rep)#self.nbin(3)
    ...
    def get_pred_ture(self,gen_val):
        I_val = gen_val.__next__()
        EC_pred,PA_pred = self.sess.run([self.EC_y,self.PA_y],feed_dict={self.seqInput:self.val_x[I_val,:,:]})#self.BS_y,self.EC_y,self.PA_
        label_true=self.val_y[I_val,:]
        EC_true,PA_true = np.hsplit(label_true,2) #(256,1)
        return  EC_pred,PA_pred,EC_true,PA_true #(256,1): 数字,不是onehot标签

    def Evalu_acc(self,repeatN,out_dir):
        print(' [*] Cal precision,recall and f1 ...')
        gen_val = self.inf_train_gen(self.dataset_num_val)
        EC_pred,PA_pred,EC_true,PA_true=self.get_pred_ture(gen_val)
        EC_true = list(np.squeeze(EC_true))
        PA_true = list(np.squeeze(PA_true))
        EC_pred = list(np.array(EC_pred))
        PA_pred = list(np.array(PA_pred))
        #labels = self.label #(128,3)#cls_predict = tf.concat((self.layers['y_BS'].expand_dims(one_img, -1),self.layers['y_EC'].expand_dims(one_img, -1),self.layers['y_PA'].expand_dims(one_img, -1)),axis=-1)#(128,) #384
        """         
