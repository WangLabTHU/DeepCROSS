...
class CLS_alone_attention():
    def __init__(self,datafile):
        self.nb_blocks = 2
        self.filters = 24
        self.BATCH_SIZE =256
        self.d_model = 128
        self.attention_num_blocks = 5
        #self.nclass=nclass
        self.data,self.charmap,self.invcharmap,self.spe_class_label,self.nclass= load_seq_data(datafile,labelflag=1)#num_class_for_oh=13 ,self.nclass
        np.random.seed(3)
        ...
        self.val_x = self.data[n_val,:,:]
        self.x = self.data[n_train,:,:]
        self.val_y = self.spe_class_label[n_val]
        self.y = self.spe_class_label[n_train]
    ...
    def PredictorNet0(self, x, is_training=True, reuse=False):
            ...
            return output#cls_logits_BS,cls_logits_EC,cls_logits_PA #cls_logits
    #########
    ...
    def PredictorNet(self, x, is_training=True, reuse=False):
        enc = Conv1D('Conv1D.1', self.c_dim, self.d_model, 13, x) #（bs,165,4）-> (bs,165,512)
        ...
        output = Dropout(0.2)(output) #保留的比例
        output = Linear('Dense_rep',41*2*self.d_model,self.d_model, output)
        #output = Bidirectional(LSTM(50,return_sequences=True))(output) #75 #(输入:bs,41,512,bs,41,100)
        #output = Linear('Dense_rep',41*2*self.d_model,self.nclass, output)#
        return output
    
    def out_rep2y(self,last2rep):
        output = Linear('Dense_out',self.d_model,self.nclass, last2rep)#
        return output
    ...
    # def get_cls_accuracy_save(self):
    ...
    def BuildModel(self,
        ...
        self.score_y = self.out_rep2y(self.out_rep)
        #1.
        self.label = tf.placeholder(tf.float32, shape=[None],name='label')
        self.score_y_argmax = tf.argmax(self.score_y, axis=-1,name='label_predict') 
        """Loss"""
        #2.
        #self.loss = tf.losses.mean_squared_error(self.label,self.score)
        self.loss = self.get_cls_loss(self.score_y,self.label)
    ...
    def get_cls_loss(self,y_logits,y_label): 
        labels=tf.cast(y_label,tf.int64)#多分类:(bs,3)
    ...
    def get_cls_accuracy(self):
        labels = self.label#y_label #self.label #self.label #(bs,1)
        cls_predict = self.score_y_argmax #y_logits #self.
        cls_predict = tf.dtypes.cast(cls_predict, tf.float32)
        labels = tf.dtypes.cast(labels, tf.float32) #128,3
        acc = tf.reduce_mean(tf.cast(tf.equal(labels, cls_predict), tf.float32)) #3 and 3
        return acc

    def save(self, checkpoint_dir,step,name='pt',acc='0'):#pt:普通save;bestacc:最好的模型保存
        ...
        self.saver.save(self.sess, os.path.join(checkpoint_dir, name+'_'+str(acc)+'_'+self.model_name +'_epoch_'+str(step)+'_'+ '.model'), global_step=step)
        
    def load(self,checkpoint_dir,model_name = None):#,step,name='pt',acc='0',
        ...
        #checkpoint_dir = os.path.join(checkpoint_dir, name+'_'+str(acc)+'_'+model_name +'_epoch_'+str(step)+'_')#+ '.model'
