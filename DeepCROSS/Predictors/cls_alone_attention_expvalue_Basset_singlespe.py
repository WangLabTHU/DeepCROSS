...
class CLS_alone_attention_expvalue_Basset_singlespe():
    def __init__(self,n=1,attention_num_blocks=5):#Nseed,
        self.nb_blocks = 2
        self.filters = 24
        self.BATCH_SIZE =256
        self.d_model = 128
    ...
    def get_cls_loss(self,EC_logits): #BS_logits, #预测的y值
        ...
        out=tf.add(EC_y_loss,PA_y_loss)/2 #out=tf.add(EC_y_loss,tf.multiply(self.w,PA_y_loss))#matmul """
        
        out = weightMSEloss(self.label,EC_logits,threshold=2)
        return out
    ...
    def get_cls_accuracy(self):
        ...
        out  = self.pearsonr_tf(self.label,self.EC_y)
        return out #[EC_acc,PA_acc] #out
    ...
    def load_dataset(self,
        ...
        PA_repeat=[]
        print('filter -2:') #-3
        print(np.array(seq_exp[0][1]))
        print(seq_exp[0][1])
        #1.过滤，只留下合理区间内的序列
        for i in range(len(seq_exp)):
            #if list(np.array(seq_exp[i][1]))>-2 and list(np.array(seq_exp[i][1]))<4:
                #组内
        ...
        for i in range(len(seqs)):
            ...
            exp=EC_norm[i]#list(np.array(seq_exp[i][1])) 
        ...
        self.y = np.array(label) # squeeze self.label_list (10185,3)
    ...
    def BuildModel(self,
        ...
        #1.
        self.label = tf.placeholder(tf.float32, shape=[None],name='label')#shape=[None,2]
    ...
    def Train(self,
        ...
        gen = self.inf_train_gen(self.dataset_num,self.Nseed)
        gen_val = self.inf_train_gen(self.dataset_num_val,self.Nseed)
        best_loss = 100000
        best_percent=0
        convIter = 0
        ...
        for epoch in range(1, 1+self.epoch):
            ...
            if loss_val < best_loss: #np.sum(cls_accuracy_val) > np.sum(best_percent): #loss_val
                ...
                self.save(self.checkpoint_dir,epoch,'bestloss',cls_accuracy_val,Nseed)
            else:
                convIter += 1
                if convIter>=earlystop:
                    print('best acc:'+str(best_percent))
    ...
    def save(self, checkpoint_dir, step,name='pt',acc=[0],Nseed=1):
        ...
        print()
        self.saver.save(self.sess, os.path.join(checkpoint_dir,name+'_acc_'+str(acc)+'_'+self.model_name +'_epoch_'+str(step)+'_seed_'+str(Nseed)+ '.model'), global_step=step)#checkpoint_dir, self.model_name + '.model'
        


    def load(self, checkpoint_dir= None, model_name = None,log_dir=None,Nseed=None):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            #
            # train_pred = self.Predictor(self.x,'oh',spe='EC')
            # val_pred = self.Predictor(self.val_x,'oh',spe='EC')
            # #EC:0;PA:1
    ...
    def Predictor(self,seq,datatype='str',spe='ECandPA'):
        if datatype == 'str':
            seq = seq2oh(seq,self.charmap)
        seq=np.array(seq) #(bs=1023, 165, 4)
        #print('seq:'+str(seq.shape))
        num = seq.shape[0] 
        batches = math.ceil(num/self.BATCH_SIZE)
        y = []
        if spe =='EC' or  spe =='EConly':
            for b in range(batches):
                y.append(self.sess.run(self.EC_y,feed_dict={self.seqInput:seq[b*self.BATCH_SIZE:(b+1)*self.BATCH_SIZE,:,:]}))
            y = np.concatenate(y)
            y = np.reshape(y,(y.shape[0]))
        elif spe =='PA'or  spe =='PAonly' or 'ECPA':
            for b in range(batches):
                y.append(self.sess.run(self.EC_y,feed_dict={self.seqInput:seq[b*self.BATCH_SIZE:(b+1)*self.BATCH_SIZE,:,:]})) #PA_y
            y = np.concatenate(y)
            y = np.reshape(y,(y.shape[0]))
        else:
            for b in range(batches):
    ...
    def get_pred_ture(self,gen_val,sameval_set=False,same_testset=None):
        EC_true=[]
        EC_pred=[]
        seq_for_merge_true=[]
        I_val = gen_val.__next__()
        #
        if sameval_set:        
            seq_exp=np.load(same_testset,allow_pickle=True)
            self.charmap={'A':0,'G':1,'C':2,'T':3}
            seq_for_merge=[]
            onehot = []
            label = []
            ...
            for i in range(len(seqs)):
                ...
                label.append(np.array(exp))
                #label=np.array(exp)
            x_sameset = np.array(onehot) #self.seq_list
            y_sameset = np.array(label) # squeeze self.label_list (10185,3)
        else:
            x_sameset = self.val_x
        EC_pred_each= self.sess.run([self.EC_y],feed_dict={self.seqInput:x_sameset[I_val,:,:]}) #@self.val_x替换成统一的数据集  EC_pred_each= self.sess.run([self.EC_y],feed_dict={self.seqInput:self.val_x[I_val,:,:]})  
        label_true=self.val_y[I_val]
        seq_for_merge_true_each = self.seq_for_merge[I_val]
        EC_true.append(label_true)
        EC_pred.append(EC_pred_each)
        seq_for_merge_true.append(seq_for_merge_true_each)
        return  EC_pred,EC_true,seq_for_merge_true #EC_pred,PA_pred,EC_true,PA_true,seq_for_merge_true #(256,1): 数字,不是onehot标签

    def Evalu_acc(self,repeatN,M,out_dir,same_testset):
        print(' [*] Cal acc ...')
        gen_val = self.inf_train_gen(self.dataset_num_val,repeatN)
        EC_pred,EC_true,seq_for_merge_true=self.get_pred_ture(gen_val,True,same_testset)
        ...
        #print('dat:') #check一下ture和pred在转置后没搞反：ok 
        #print(dat)
        datM_predsort = dat[np.argsort(-dat[:,0])] #第0列是pred，第1列是ture
        print('datM_predsort datframe:')
        print(datM_predsort)
        print('datM_predsort true max:')
        print(max(datM_predsort[:,1]))
        #NDCG
        NDCG = self.cal_NDCG(datM_predsort)
        #
        datM_predsort_Mtrue = datM_predsort[:M,1]#[:M,:]
        #max
        max_topM = min(1,np.max(datM_predsort_Mtrue/max(datM_predsort[:,1])))#不是排序后，前M个最大，而是所有最大
        #mean
        mean_topM = np.mean(datM_predsort_Mtrue/max(datM_predsort[:,1]))
        print('M='+str(M)+', max_topM: '+str(max_topM),', mean_topM:'+str(mean_topM),'NDCG:'+str(NDCG))
        #
        #self.load()
        return mean_topM,max_topM,NDCG

    def cal_NDCG(self,datM_true):
        pred_rank_M = datM_true
        true_rank_M = datM_true[np.argsort(-datM_true[:,1])]
        up=0
        down=0
        for i in range(len(pred_rank_M)):
            up+=pred_rank_M[i,1]/np.log2(i+2) #up：pred排序降序排列的EC_ture
            down+=true_rank_M[i,1]/np.log2(i+2) #up：true排序下降序排列的EC_ture
        print(up)
        print(down)
        out = up/down
        return out
